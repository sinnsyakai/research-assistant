...
Example: "マスク Twitter" → "イーロン・マスク Twitter"`;
        ...
        if (isGlobalMode && apiKey) {
            ...
            try {
                ...
                const translationPrompt = `You are an expert translator helping a Japanese user search on US Google.

Your task: Convert this Japanese search query into the BEST English search terms to find relevant results.

CRITICAL INSTRUCTIONS:

1. UNDERSTAND THE MEANING AND INTENT - not literal translation
2. EXPAND ABSTRACT CONCEPTS TO CONCRETE EXAMPLES:
   - "SNS" or "ソーシャルメディア" → Include specific platforms: "TikTok Instagram YouTube Twitter"
   - "スイーツ" → Include specific types: "desserts cakes pastries trending sweets"
   - "睡眠障害" → Include specific conditions: "insomnia sleep apnea sleep disorders"
   - Generic categories should be expanded to specific popular examples

3. USE PROPER ENGLISH TERMINOLOGY:
   - Medical terms: 性機能 → "sexual function", 睡眠障害 → "sleep disorders"
   - Tech terms: Use industry-standard English
   - For news queries: Add "news" or "latest" or "2024 2025"

4. THINK ABOUT SEARCH INTENT:
   - If searching for "SNSニュース", user wants news ABOUT TikTok, Instagram, etc.
   - If searching for "睡眠障害", user wants info about various sleep problems
   - Include the umbrella term AND specific examples

Japanese Query: "${clarifiedQuery}"

Return the BEST English search query (include specific examples if the query is abstract).
...
Return ONLY the optimized English search query:`;
            ...
            // Filter: has abstract only (removed country restriction for better results)
            url.searchParams.set('filter', 'has_abstract:true');
            // Sort by relevance_score for better matching
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    let papers = data.results.map((work: any) => ({
                        paperId: work.id,
                    ...
                    if (apiKey && papers.length > 0) {
                        ...
                        if (englishPapers.length > 0) {
                            try {
                                ...
                                const papersToTranslate = englishPapers.slice(0, 15);
